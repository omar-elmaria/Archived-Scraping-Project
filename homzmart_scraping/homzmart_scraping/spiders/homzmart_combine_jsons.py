...
path_to_json = '/home/oelmaria/python_projects/homzmart_project/homzmart_scraping'
...
# df_home_cat = df_home_page.merge(df_cat_page, left_on = 'cat_url', right_on = 'response_url', how = 'right', suffixes = ('_home', '_cat')) # RIGHT JOIN as the right DF is larger
df_home_cat = df_cat_page.merge(df_home_page, left_on = 'response_url', right_on = 'cat_url', how = 'left', suffixes = ('_cat', '_home')) # RIGHT JOIN as the right DF is larger
...
df_cat_subcat_lod = df_home_cat.replace(np.nan, None, regex = True).to_dict(orient='records') # All categories and sub-categories
df_prod_links_lod = df_subcat_page.replace(np.nan, None, regex = True).to_dict(orient='records') # All individual product links under each scraped sub-category
df_pdp_info_lod = df_prod_page.replace(np.nan, None, regex = True).to_dict(orient='records') # All PDP info of each scraped product link under the scraped sub-categories
df_all_lod = df_all.replace(np.nan, None, regex = True).to_dict(orient='records') # All information combined
